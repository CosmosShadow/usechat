...
> UseChat 是一个本地优先的消息软件 Computer Use 运行时：它把微信桌面端变成可读、可写、可观察、可审计的本地工具接口，让 Codex、Claude Code、Cursor、OpenCode 和任意自定义 Agent 都能在用户授权的可见桌面里处理消息。
...
UseChat 需要一个支持视觉输入的 OpenAI-compatible 模型，用于把微信窗口截图结构化成消息列表。
...
使用其他 OpenAI-compatible endpoint 时，只需要替换 `baseUrl` 和 `model.name`：
