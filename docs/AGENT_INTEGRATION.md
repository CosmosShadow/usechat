...
1. 直接调用 CLI：`usechat doctor`、`usechat read`、`usechat write`、`usechat watch`。
...
`watch` 模式同样只包装已经 copy-out 的 read runtime 与 ledger：它按固定间隔读取指定对话，用本机 ledger 建立 baseline 并去重，然后输出 JSONL 事件。
...
- `--poll-interval-ms <ms>`：轮询间隔，沿用 Shennian 范围，会 clamp 到 30 秒至 5 分钟。
- `--limit <n>`：每次 read 的可见消息限制。
- `--download never|auto`：是否尝试物化当前可见媒体。
- `--once`：只执行一轮 tick，主要用于测试和 smoke。

事件类型：

```json
{"type":"baseline","app":"wechat","chat":"ABC","observedCount":20,"revision":0}
{"type":"message","app":"wechat","chat":"ABC","stableMessageKey":"...","message":{}}
{"type":"error","app":"wechat","chat":"ABC","reasonCode":"wechat_login_required","message":"..."}
{"type":"paused","app":"wechat","chat":"ABC","reasonCode":"wechat_login_required","nextPollAt":"..."}
```

第一轮 `baseline` 只建立本机 ledger，不回放历史可见消息；后续 `message` 只输出 ledger 判定为新且可投递的消息。
...
| CLI `watch` | `usechat_watch` / resource stream | 观察消息事件流 |
