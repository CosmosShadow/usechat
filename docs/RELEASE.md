...
## Helper artifact 分发策略

Helper runtime 体积大，包含 macOS app bundle、Windows self-contained runtime、native DLL 和 PP-OCRv5 模型。它们不进入 git 代码库，也不塞进 npm package：

- git 代码库只保留源码、manifest 输入、构建脚本和校验脚本；
- npm `@shennian/usechat` 只发布 JS CLI / SDK，不带 GUI helper，也不在 `postinstall` 静默下载；
- helper zip 发布到 GitHub Releases（必要时再镜像到 `shennian.net` / R2 / S3 CDN）；
- 用户通过 `usechat setup-helper --from <zip>` 或后续 `usechat setup-helper --download` 显式安装；
- 下载/安装前必须校验 platform、version、sha256、manifest 和签名状态。
...
Helper 对外名称统一为 `UseChat Helper` / `UseChat Helper.app`。首个 UseChat release 只保持 helper protocol 和 response shape 与 copy-out 来源兼容，不再对外暴露旧 Helper app / runtime 目录名。
...
正式执行前需要确认 npm 登录账号具有 `@shennian/*` scope 的 publish 权限，并保留 provenance / CI 构建日志。
...
公开 release artifact 必须在 evidence 中明确标记签名、notarization 和 SmartScreen / Authenticode 状态。公开仓库前必须完成：
