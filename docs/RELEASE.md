...
UseChat 当前是私有正式项目，release 目标是先服务内部测试用户。微信能力必须来自 Shennian copy-out 的源码、脚本和运行时资产；release 脚本只负责构建、打包、校验和记录 provenance，不重新实现微信 RPA。

## Release 产物

一次私有 release 至少包含两类产物：

1. npm / GitHub Packages 内部包：
   - `@shennian/usechat-core`
   - `@shennian/usechat-model-provider`
   - `@shennian/usechat-sdk`
   - `@shennian/usechat`
2. native helper runtime：
   - macOS：`Shennian Helper.app` + `helper-runtime-package.json` + `helper-runtime-evidence.json` + zip。
   - Windows：`Shennian Helper` runtime 目录 + `install-helper-runtime.ps1` + `helper-runtime-package.json` + `helper-runtime-evidence.json` + zip。
...
- 内部 npm registry 或 GitHub Packages scope：`@shennian/*`。
...
helper-runtime/dist/macos/
  Shennian Helper.app/
  Shennian-Helper-Runtime-macos.zip
...
helper-runtime/dist/windows/
  Shennian Helper/
  Shennian-Helper-Runtime-windows.zip
...
1. 安装 Node.js / pnpm。
2. 从内部 registry 或 tarball 安装 `@shennian/usechat`。
